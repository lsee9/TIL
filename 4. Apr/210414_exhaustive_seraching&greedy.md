...
- 다수의 알고리즘 문제들은 순서화된 요소들의 집합에서 최선의 방법을 찾는 것과 관련이 있다
  ...
  - 예) TSP (Traveling Salesman Problem)
    ...
    - 도시 방문 순서를 잘 정하면 비용을 줄일 수 있다!!! 라는 문제

    - 어떤 순서로 방문해야할까???

      ###### 모든 순서를 만들고, 최소인 경우를 찾쟈! 라는 문제!

- **n개의 요소**들에 대해 **n!개의 순열**들이 존재!!!!

  - 만약 방문할 도시가 **12개**라면..?

    ###### 12! = 479,001,600 

  - n > 12 인 경우, 시간 복잡도 폭발적으로 증가!! :bomb:

#### 순열의 경우의 수

표를 넣자

- 어쨌든.. 경우의 수가 급격히 커진다는 것@@@
...
for idx1 in range(1, N):
    for idx2 in range(1, N):
        if idx2 != idx1:  #앞의 것과 다른 경우만 선택
            for idx3 in range(1, N):
                if idx3 != idx1 and idx3 != idx2:
                    print(arr[idx1], arr[idx2], arr[idx3])
```

- 이정도는 생각할 수 있어야겠죠???
- 만약 같은 크기의 순열을 여러번 생성해야 한다면, 미리 만들어놓고 가져와 쓰면되겠습니다 (뭐 인덱스로 저장해놓는다던가...?)
